...
from tests.factories import CoffeeFactory, UserFactory
...
class CoffeeUser(FastHttpUser):
    ...
    def create_coffee(self):
        self.client.post(
            url="/coffee/",
            json=json.loads(CoffeeFactory().build().json()),
            name="create_coffee",
        )
    ...
    def update_coffee(self):
        self.client.put(
            url=f"/coffee/{random.randint(1, 100)}",
            json=json.loads(CoffeeFactory().build().json(exclude_unset=True)),
            name="update_coffee",
        )
...
class UsersUser(FastHttpUser):
    ...
    def create_user(self):
        self.client.post(
            url="/users/",
            json=json.loads(UserFactory().build().json()),
            name="create_user",
        )
    ...
    def update_user(self):
        self.client.put(
            url=f"/users/{random.randint(1, 100)}",
            json=json.loads(UserFactory().build().json(exclude_unset=True)),
            name="update_user",
        )
